...
exisd <- versions[,{
    ...
    pp[, {
        ...
        data.table(npre      =isn(ndays[w0],0),
                  ...
                  srchpoall  = isn(sum(search[wall]),0),
...
h <- exisd[ version %in% c(57,56),]
h[, mean(actpo2),by=list(version, shc)][order(shc,version),]
```
...
```{r}
h <- exisd[ version %in% c(57,56),]
m1 <- glm(actpo2 ~ version+log(1+shpre),data=h, family='binomial')
