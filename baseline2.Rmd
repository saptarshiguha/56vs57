...
v2 <- versions[, {
    ...
    s2 <- .BY$date+6
    V <- .BY$version
    list(j = d0[pcd >=s1 & pcd<=s2,][, list(v = min(version[date>=pcd & date<=(pcd+1)])) ,by=cidf][, sum(v<V)/.N*100])
...
pl <- function(v,s1,main,cutOut=1,xaxs='pcd',xlab='Profile Creation Date'){
    xyplot(get(v)  ~ get(xaxs), xlab=xlab
         , ylab=''
         , type=c('l')
...
```{r baselineExist,eval=FALSE,cache=TRUE}
d01 <- d0[date>='2017-01-01',d0[sh<=24 & pcd<=(.BY$date-13) & date>=(.BY$date) & date<=(.BY$date+6),
                                list(sh=sum(sh,na.rm=TRUE)/.N,
                                     ...
                                     search=sum(search)/(sum(sh,na.rm=TRUE)+C),
...
```{r searchesbase2,cache=TRUE,dependson=c('makePlot','newOn'),fig.align='center',fig.width=8}
e2 <- d01[,list(date=date,s=search)][order(date),]
pl("s",e2,"Searches made /hour/profile",xaxs='date',xlab='Date ',cut=0.995)
```
...
pl("s",e2,"CRash Rate (Definition 2)",xaxs='date',xlab='Date ',cut=0.995)
```
