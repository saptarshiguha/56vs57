...
def newprofiles(version,minpcd,maxpcd,minsubmission,maxsubmission,minsubsession,maxsubsession,doall=False):
    st= """
    SELECT
    client_id as cid,
    'new' as persona,
...
cohort54 = newprofiles('54', 
                       minpcd=(releases['54']-releases['origin']).days,
                       maxpcd=(releases['54']-releases['origin']).days+ACQPERIOD,
                       minsubmission=releases['54'].strftime("%Y%m%d"),
                       maxsubmission=(releases['54']+datetime.timedelta(days=DURA+ACQPERIOD+LAGBUFF)).strftime("%Y%m%d"),
                       minsubsession=releases['54'].strftime("%Y-%m-%d"),
                       maxsubsession=(releases['54']+datetime.timedelta(days=DURA+ACQPERIOD)).strftime("%Y-%m-%d"))
...
cohort55all = newprofiles('55all', 
                       minpcd=(releases['55']-releases['origin']).days,
                       maxpcd=(releases['55']-releases['origin']).days+ACQPERIOD,
                       minsubmission=releases['55'].strftime("%Y%m%d"),
                       maxsubmission=(releases['55']+datetime.timedelta(days=DURA+ACQPERIOD+LAGBUFF)).strftime("%Y%m%d"),
                       minsubsession=releases['55'].strftime("%Y-%m-%d"),
                       maxsubsession=(releases['55']+datetime.timedelta(days=DURA+ACQPERIOD)).strftime("%Y-%m-%d"),doall=True)
...
cohorts = cohort57[1].union(cohort56[1].union(cohort55[1].union(cohort55all[1].union(cohort54[1])  )))
