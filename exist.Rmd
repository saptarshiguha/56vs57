...
def base(minSubmission, maxSubmission,minsubsession,maxsubsession,sampleid):
    a= spark.sql("""
    select 
    client_id as cid,
    from_unixtime(unix_timestamp(substr(subsession_start_date,1,10),'yyyy-MM-dd'),'yyyy-MM-dd') as date,
    ...
    group by 1,2,3,4
    """.format( minSubmission=minSubmission, maxSubmission=maxSubmission,maxsubsession=maxsubsession,minsubmission=minsubmission,
                sampleid=sampleid))
...
v54 = prePost('42',toVersion="54",toVersionLabel="54",
              ...
              minsubsession=(releases['54']+datetime.timedelta(days=-14)).strftime("%Y-%m-%d"),
              maxsubsession=(releases['54']+datetime.timedelta(days=14+21)).strftime("%Y-%m-%d"),
              dbefore=14,dafter=21)
...
max(case when daysSinceUpdate >=0 then active else 0 end) as activeAll,
max(case when daysSinceUpdate >=0 and daysSinceUpdate<7 then active else 0 end) as active7,
max(case when daysSinceUpdate >=7 and daysSinceUpdate<14 then active else 0 end) as active14,
max(case when daysSinceUpdate >=14 and daysSinceUpdate<21 then active else 0 end) as active21
from cohorts
group by 1,2
""")
...
sum(case when daysSinceUpdate <0 then th else 0 end) as th,
sum(case when daysSinceUpdate <0 then ah else 0 end) as ah,
max(case when daysSinceUpdate <0 then crq else 0 end) as crashedpreAll,
max(case when daysSinceUpdate >=0 and daysSinceUpdate<7 then crq else 0 end) as crashedpost7,
max(case when daysSinceUpdate >=0 and daysSinceUpdate<14 then crq else 0 end) as crashedpost14
from cohorts
group by 1,2
""")
